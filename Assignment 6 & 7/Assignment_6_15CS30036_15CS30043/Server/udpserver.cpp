...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    while(bytesrec < files)
    {
      ...
      //printf("Expected seq number - %d ", seq);
      ...
      {
        printf("Receiving %d Bytes - %d \n", data_bytes_read, seqrec);
        bzero(buf, BUFSIZE);
        ...
        //Dropping acknowledgements with a certain probability 
        if(ran < PROB && seq!=packs)
        {
